...
Fichiers créés à la racine du projet : `docker-compose.yml`, `prometheus/prometheus.yml`, `grafana/provisioning/datasources/victoriametrics.yml`, `.env.example`, `.gitignore`, `README.md`. **Voir `README.md` §1 pour le schéma complet à jour (diagramme Mermaid) et le détail de chaque service** — ne pas dupliquer ici.

Services actuels, tous sur un réseau Docker dédié `domotique_net` : `homeassistant`, `glasshome`, `prometheus`, `victoriametrics`, `grafana`, `doc-knx`, `tesla-key`, `teslamate-db`, `teslamate-mosquitto`, `teslamate`, `cloudflared` — plus `Tunet`, dashboard existant sur un réseau Docker séparé, joint via `host.docker.internal`. `dashboard-api`/`dashboard-web` (voir `dashboard/CAHIER_DES_CHARGES.md`) viendront s'y ajouter une fois construits.
...
- 2026-09-07 — Schéma d'architecture global (README.md §1) refait en diagramme Mermaid, mis à jour pour couvrir toute la stack réelle (auparavant seuls homeassistant/prometheus/victoriametrics/grafana/cloudflared y figuraient) : glasshome, doc-knx, tesla-key, la pile TeslaMate (teslamate + teslamate-db + teslamate-mosquitto), Tunet (réseau Docker séparé), et les 5 sous-domaines Cloudflare existants (domotiquebulle, grafanabulle, docbulle, vehiculebulle, visubulle). Ajout de la couche `dashboard-api`/`dashboard-web` (projet dashboard sur-mesure, suivi dans `dashboard/CAHIER_DES_CHARGES.md`) comme brique **prévue, pas encore construite**, connectée à Home Assistant de la même manière que GlassHome. §7 mis à jour pour renvoyer vers ce schéma plutôt que de dupliquer la liste des services.
...
- 2026-09-10 — Faille de sécurité identifiée : `dashboardbulle.malnoy.com` (prototype `dashboard-proto`, exposé publiquement depuis le 08-09.09.2026) accessible sans authentification, contrairement à `docbulle.malnoy.com` (déjà protégé par Cloudflare Access). Décision : généraliser Cloudflare Access à tous les sous-domaines sensibles — `domotiquebulle`, `grafanabulle`, `visubulle`, `dashboardbulle` — avec la même politique que `docbulle` (email `fabrice@malnoy.com`, session 24h). Exception maintenue pour `vehiculebulle.malnoy.com` : les serveurs Tesla doivent pouvoir lire la clé publique sans authentification. Décision documentée dans `README.md` §4.6 (étapes manuelles à suivre dans le tableau de bord Cloudflare Zero Trust) et `dashboard/CAHIER_DES_CHARGES.md` §5.3/§12 ; résout le point ouvert §12.4. Mise en œuvre côté Cloudflare non encore effectuée — à faire par l'utilisateur, Claude ne pouvant pas modifier des réglages de sécurité.
- 2026-09-11 — Page d'accueil de `docbulle.malnoy.com` refaite (bandeau photo + illustration de coupe de la maison entourée de 4 cartes : README, architecture, bus KNX, feuille de route), identité visuelle Villa Bulle (palette Nuit/Ambre/Glacier/Mousse/Cuivre, Fraunces/Inter/IBM Plex Mono), validée par l'utilisateur d'abord en prototype (Claude Artifact) avant déploiement. Même convention que `docs_site/knx/` : contenu régénéré depuis les sources (`README.md`, `docker-compose.yml`, `dashboard/CAHIER_DES_CHARGES.md`), jamais édité à la main — pipeline documenté dans `docs_site_src/README.md` (nécessite mermaid-cli/Node/Python, absents du Mac mini ; à relancer en redemandant à Claude). Photo de bannière (recadrage de `maison-facade-jardin-2.jpg`) servie en asset statique `docs_site/assets/villa-bulle-banner.jpg` plutôt qu'encodée en base64, pour garder le fichier `docs_site/index.html` léger (~150 Ko).
- 2026-09-11 — Illustration de la maison enrichie à la demande de l'utilisateur : les 3 nœuds génériques du bus KNX remplacés par des bulles d'icônes connectées (stores, prises électriques, véhicule), inspirées d'une image de référence fournie par l'utilisateur (maison + icônes reliées par des traits). Pas d'outil de génération d'image dans cette session (vérifié : ni outil dédié, ni connecteur MCP disponible) — reproduit à la place en SVG vectoriel dans le style déjà en place (thème clair/sombre, bus animé), plutôt qu'une image statique.
- 2026-09-11 — Diagramme d'architecture logicielle refait : l'utilisateur l'a jugé illisible et pas « state of the art » (rendu mermaid-cli à thème sombre figé, illisible en mode clair), puis a demandé une vue macro avec descente dans l'arborescence au clic sur les blocs principaux, et un popup de dépendances au clic sur chaque service. Remplacé par un système à trois niveaux généré en SVG pur Python (`docs_site_src/scripts/architecture_data.py` pour les données services/dépendances, `architecture_view.py` pour le rendu) : (1) grille macro de 7 zones fonctionnelles cliquables, intégrée aussi en résumé dans la vue README ; (2) détail par zone avec mini-diagramme (nœuds internes + bulles de dépendances croisées cliquables vers les autres zones) ; (3) popup JS (« Dépend de » / « Utilisé par ») au clic sur un service, depuis `window.ARCH_DATA`. Thémé avec les mêmes variables CSS que le reste du site (`var(--ambre)` etc.) donc lisible en clair comme en sombre, contrairement au rendu mermaid précédent. mermaid-cli/Node/Chromium entièrement retirés du pipeline (`mermaid-theme.json`, `puppeteer-config.json`, `architecture_diagram.py` supprimés) : la régénération ne dépend plus que de Python (`markdown`, `Pillow`), toujours à relancer en redemandant à Claude (voir `docs_site_src/README.md`, mis à jour).

- 2026-09-11 — Diagramme d'architecture remplacé une seconde fois le même jour : l'utilisateur a fourni une image de référence (infographie « Infrastructure Villa Bulle », swimlanes par domaine) et a demandé un regroupement par domaine fonctionnel plutôt que par réseau Docker, avec Home Assistant seul au centre du domaine Domotique (la solution matérielle, un Mac mini, devant apparaître), et un domaine KNX distinct pour le bus filaire + configuration ETS. Décision de l'utilisateur (3 questions posées) : remplacer aussi la vue Architecture (pas seulement le résumé README), représenter le Mac mini comme un grand cadre englobant les domaines hébergés en Docker, et le KNX comme une case avec ses chiffres clés (975 adresses · 59 fonctions · 19 pièces) plutôt qu'un simple lien. Le système à trois niveaux (macro/détail/popup, `architecture_view.py`) de la décision précédente est donc abandonné après quelques heures — remplacé par une infographie SVG statique unique (`architecture_overview.py`) : plus de navigation ni de popup JS, tout le schéma visible d'un coup, réutilisé tel quel en haut du README et dans la vue Architecture logicielle. Domaines : Domotique (Home Assistant seul), Monitoring (Prometheus/VictoriaMetrics/Grafana), Documentation (doc-knx), Dashboard (GlassHome + Tunet + dashboard-proto/api/web, régroupés — auparavant répartis entre Domotique et Externe), Véhicule (Tesla), Accès distant (cloudflared). Bus KNX filaire représenté hors du cadre Mac mini (installation physique, pas un service Docker). Connecteurs routés à la main par couloirs verticaux/horizontaux vérifiés libres de tout cadre (pas de routeur automatique). `architecture_view.py` supprimé ; `docs_site_src/README.md` mis à jour pour décrire le nouveau système.

- 2026-09-13 — Page « Inspirations » ajoutée à docbulle.malnoy.com (6ᵉ tuile de l'accueil, accent violet) : reprend, sur le même principe que la vue Écran Énergie (12.09.2026), toutes les images envoyées en exemple (style, dashboard, photos) au fil des discussions du dashboard sur mesure depuis fin août 2026, organisées par date/décision (pas de correspondance numérotée fiable avec l'inventaire du 31.08.2026), avec le commentaire fait au moment de chaque envoi. 20 images retenues parmi ~45 candidates repérées dans `dashboard/assets/`, recadrées/compressées selon la convention `energie-inspi-*.jpg` déjà en place (`docs_site/assets/inspi-*.jpg`, ~1400px, qualité 82%). Deux références du 11.09.2026 documentées en texte seul, sans image conservée. Perte de données confirmée dans la mémoire projet `custom_dashboard_10092026_meteo_graphes.md` (contenu remplacé par une commande shell non exécutée, détail des 4 styles de mini-graphes de température irrécupérable) — documentée honnêtement dans la page plutôt qu'inventée. `docs_site_src/{content.py,build_index.py,template.html,README.md}` mis à jour (source de vérité, jamais éditée à la main dans `docs_site/`), page régénérée et déployée, vérifiée responsive (≤480px) et en thème clair/sombre avant déploiement.

- 2026-09-18 — Les 3 graphes de l'écran Énergie du dashboard sur mesure (`dashboard_web_nginx/index.html`) s'agrandissent désormais au clic (bouton ⤢ dans l'en-tête de chaque carte : flux en direct, courbe de consommation, donut de répartition), demande de l'utilisateur ("les graphiques sont trop petits"). Mécanisme générique (`openCardZoom`/`closeCardZoom`) : la carte cliquée est déplacée telle quelle dans un panneau plein écran (pas clonée, pour garder ses listeners et, pour le flux, sa boucle d'animation requestAnimationFrame), puis replacée à sa position d'origine à la fermeture (bouton ×, clic hors panneau, ou touche Échap). Bug rencontré et corrigé en route : le panneau, d'abord placé à l'intérieur de `.view-block`/`.main-area` (z-index:4), restait cliqueable mais son bouton de fermeture se retrouvait sous la barre latérale météo (`.bar`, z-index:5) — un enfant ne peut jamais dépasser en z-index le contexte d'empilement de son parent. Corrigé en plaçant le panneau en frère direct de `.main-area`/`.bar-left`/`.bar-right` sous `#screen`. Sauvegarde du fichier faite avant modification (conforme §6) ; vérifié en local via Playwright (ouverture/fermeture des 3 cartes par les 3 méthodes, animation du flux toujours active une fois agrandie, mise à l'échelle correcte sur petit viewport) avant déploiement — fichier servi en lecture seule par nginx (`dashboard_web_nginx`, port 8092), donc live immédiatement sans redémarrage.

- 2026-09-20 — Écran Configuration du dashboard sur mesure (`dashboard_web_nginx/index.html`) : (1) section « Fond d'écran » déplacée en haut à droite, désormais côte à côte avec « Style des cartes » (nouveau conteneur `.cfg-row-top`) au lieu d'être la dernière section tout en bas — demande de l'utilisateur. (2) Ajout de la taille de police (en plus du choix déjà existant de la police elle-même) pour les 2 mêmes catégories Titres/Valeurs de la section Typographie, avec 6 valeurs prédéfinies (80/90/100/110/120/130%, défaut 100%) — demande de l'utilisateur. Mécanisme : 2 nouvelles variables CSS `--fs-title-scale`/`--fs-value-scale` (défaut 1), appliquées en réécrivant chaque règle `font-size` déjà taguée `var(--ff-title)`/`var(--ff-value)` (43 règles au total, 15 titres/28 valeurs) en `calc(valeur * var(--fs-*-scale))` — repère la même convention que le choix de police (18-19.09.2026). Exception volontaire : les 4 éléments d'axe de graphe (`.tc-label`/`.tc-ylabel`/`.enr-chart-axis`/`.enr-usage-axis`) restent uniquement sur `--gr-axis-size` (réglage Graphes du 19.09.2026) pour ne pas cumuler les deux échelles. Entièrement en CSS pur (`calc()`), contrairement au réglage Graphes : aucun redessin JS nécessaire (confirmé qu'aucune taille de police titre/valeur n'est écrite en dur dans le JS générant les graphes). `setFontSize()`/`initFontSize()` ajoutées sur le même modèle que `setFontChoice()` (persistance `localStorage`, clés `villaBulle.fontTitleSize`/`villaBulle.fontValueSize`, tolère le mode privé). Sauvegardes faites avant chaque étape (conforme §6) ; vérifié en local via Playwright (positionnement de la ligne du haut, clic sur chaque taille pour les 2 catégories, valeur `--fs-*-scale` et taille réellement rendue, non-régression des axes de graphe, persistance `localStorage`) avant déploiement — fichier servi en lecture seule par nginx, donc live immédiatement.

- 2026-09-22 — Design retenu pour les cartes thermostat du futur écran **Température** du dashboard sur mesure (`dashboard_web_nginx/index.html`, entrée de navigation déjà présente mais écran encore "placeholder", pas construit) : décision prise dans une conversation Claude dédiée ("Thermostat"), aboutie à la 21e itération ("canvas 21"), documentée ici sur demande de l'utilisateur à partir de l'Artifact final (`https://claude.ai/artifact/VCPHdXMrNdNa92PzECXhzH`, type Design/canvas, titre "Thermostat Embrasement"). Détail complet consigné dans `dashboard/CAHIER_DES_CHARGES.md` §9 (Maquettes visuelles livrées) et §8 (Système visuel, point ouvert sur le thème). Résumé : cadran circulaire neumorphique (relief par ombres uniquement), 4 paliers d'intensité de chauffe + arrêt matérialisés par un halo de braise animé, un liseré lumineux, une teinte de disque et un anneau de "flamme" qui s'intensifient avec le palier ; arc de progression = pourcentage d'ouverture de vanne ; consigne + température actuelle au centre ; bouton d'historique (consigne/actuelle/vanne) remplacé par un bouton d'alerte pulsant en cas de défaut thermostat ; 4 boutons de mode KNX (Hors-gel/Nuit/Éco/Confort) sous le cadran. Le canvas illustre les 5 états avec des pièces réelles (dont Chambre Léane pour l'état défaut, seule pièce disposant aujourd'hui d'un `binary_sensor` Erreur Chauffage côté KNX/HA). **Point ouvert non résolu par cette décision** : le canvas est composé sur fond clair/neumorphique, alors que le reste du dashboard assume un thème sombre unique (`CAHIER_DES_CHARGES.md` §8) — à trancher au moment de construire réellement l'écran. Décision purement visuelle/documentaire à ce stade : aucune implémentation dans `dashboard_web_nginx/index.html` (l'utilisateur a explicitement demandé de documenter la décision, pas de construire l'écran).

- 2026-09-24 — Écran **Température** du dashboard sur mesure effectivement construit (contrairement à l'entrée du 22.09.2026, écrite avant) : grille 3×5 par étage, 7 thermostats KNX réels + 8 emplacements « non raccordé », cadran « Embrasement » v32 porté du canvas (graduations illuminées jusqu'à la consigne sur une échelle 15–25 °C, boutons −/+ encore décoratifs, anneau retiré). Version v55 de l'artifact « Villa Bulle — Accueil », vérifiée identique à l'octet près à `dashboard_web_nginx/index.html` sur le Mac mini, contrôlée visuellement via Playwright (2560×1440, 144 graduations allumées, aucune erreur JS) puis commitée (`de78e5a`).
