...
4. Protection d'accès au dashboard depuis internet : réseau local + authentification existante, ou Cloudflare Access dédiée (§5.3) ?
5. Comment intégrer précisément les effets de Concept A/B (parallax, respiration, panneau coulissant) dans le gabarit Boussole retenu (§9) ?
6. Devenir des anciennes entrées Extérieur/Énergie/Tesla/Sécurité/Fonctions dans la navigation gauche (T9, désormais 7 entrées : Accueil/Pièces/Lumière/Température/Energie/Météo/Configuration) — fondues dedans, gardées à part, ou autre ? Météo a déjà sa propre entrée (ajoutée le 06.09.2026) et n'est donc plus concernée par cette question. Pas urgent.
7. Comment gérer précisément les étages maintenant que leur sélecteur est intégré à l'écran Pièces (E0) plutôt que dans une colonne dédiée ? Pas urgent, à revoir plus tard (confirmé par l'utilisateur le 06.09.2026).
8. L'artifact interactif "Boussole" (§9) reflète encore l'ancien gabarit (rail droit = étages) et n'a pas de photo sur son écran d'accueil — à resynchroniser avec le T9 révisé une fois les points 6-7 clarifiés, en même temps que l'ajout de la photo hero (§3.2, A4) — "Villa Bulle — Accueil" (§9) sert désormais de référence pour cette resynchronisation.
9. Icônes de navigation (T9) et icônes d'état lampe/chauffage/volet (A9, E1/E2) — le principe (Lucide, libre/gratuit, choix en parcourant la bibliothèque réelle) est acquis depuis le choix des icônes de pièces (§7.1), reste à répéter la même démarche pour celles-ci.
