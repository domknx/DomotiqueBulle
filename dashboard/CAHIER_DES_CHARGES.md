...
- **Jauge** (`#accEcoGauge`/`#accEcoGaugeFill`) : piste verticale pleine hauteur (52px de large), remplissage dont la **hauteur** ET la **couleur** reflètent le pourcentage — 100% = jauge pleine et verte ("tout vert" si l'énergie consommée vient de la batterie et du solaire), 0% = jauge presque vide et rouge ("complètement rouge" si elle vient uniquement du réseau), toute valeur intermédiaire = jauge partiellement remplie dans une teinte orangée ("en partie rouge" si elle vient du réseau). Dégradé de couleur continu (pas de paliers), interpolé en RGB par `pfcEcoColor()` sur 3 arrêts : rouge `--rouge` (0%) → ambre `--ambre` (50%) → vert `--mousse` (100%).
